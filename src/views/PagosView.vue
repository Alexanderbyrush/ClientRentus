  ...
  <div class="contracts-page">
    ...
    <main class="carousel-container">
      ...
      <div v-else class="carousel">
        <div v-for="(contract, index) in contracts" :key="contract.id" class="card payment-card" :class="{
          active: activeIndex === index,
          [getContractStatusClass(contract.payment_status)]: true,
        }" @click="setActiveContract(index)">
          <div class="card-badge" :class="getBadgeClass(contract.payment_status)">
          ...
          <div class="card-actions">
            <button v-if="contract.payment_status === 'pending'" class="btn-pay"
              @click.stop="openPaymentModal(contract)">
              <i class="fas fa-money-bill-wave"></i>
              Realizar Pago
            </button>

            <button v-else-if="contract.payment_status === 'paid'" class="btn-receipt-sm"
              @click.stop="viewReceipt(contract.latest_payment_id)">
              <i class="fas fa-file-invoice"></i>
              Ver Comprobante
            </button>

            <button v-else class="btn-contact-sm" @click.stop="viewReceipt(contract.latest_payment_id)">
    ...
    <PaymentGateway v-if="showPaymentModal" :amount="currentContract.amount_due" :order-id="currentContract.id"
      :reference="paymentReference" @close="showPaymentModal = false" @payment-success="handlePaymentSuccess" />
    ...
    <div v-if="showSuccessModal" class="success-modal-overlay-dark" @click="showSuccessModal = false">
      <div class="success-modal-content-dark">
        <svg class="success-icon-dark" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
